import { a_fadeIn, a_fadeOut, a_AnimateLeft } from "./jq_animation.js";
...
const setActiveLink = (href) => {
    $(`.mobile_menu_nav_link_main[href='${href}']`).addClass(
        "mobile_menu_nav_link_main_active"
    );
};
...
// Phone popup
{
    ...
    class Tooltip {
        constructor(el) {
            ...
            this.DOM.triggerSpan = this.DOM.el.querySelector(
            ...
            this.DOM.letters =
                this.DOM.content.querySelector(".tooltip__letters");
            if (this.DOM.letters) {
        ...
        animate(dir) {
            ...
            if (config[this.type][dir].shape) {
                ...
                anime(
                    Object.assign(shapeAnimOpts, config[this.type][dir].shape)
                );
            }
            if (config[this.type][dir].path) {
                ...
                anime(
                    Object.assign(shapeAnimOpts, config[this.type][dir].path)
                );
            }
...
pageHeight += parseInt(
    window.getComputedStyle(pageMobileBlock).getPropertyValue("margin-top")
);
pageHeight += parseInt(
    window.getComputedStyle(pageMobileBlock).getPropertyValue("margin-bottom")
);
...
const hamburgerFix = (isOpening) => {
    if (isOpening) {
        document.querySelector(".header_hamburger").style.pointerEvents =
            "none";
