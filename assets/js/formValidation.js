...
const labelShift = (id) => {
    document.getElementById(id).addEventListener('input', (e) => {
        if (e.target.value.length > 0) {
            e.target.nextElementSibling.classList.add('contact_row_form_block_item_label_active')
        } else {
            e.target.nextElementSibling.classList.remove('contact_row_form_block_item_label_active')
        }
    });
}
...
const submitHandler = () => {
    ...
    const isServices = $('#select_name_text') !== undefined ? $('#select_name_text').text().trim() : '';
    const isServicesType = $('#select_type_text') !== undefined ? $('#select_type_text').text().trim() : '';
    ...
    if (isCanSubmit === true) {
        ...
        for (var i = 0; i < list.length; ++i) {
            list[i].classList.remove('contact_row_form_block_item_label_active');
        }

        if (document.querySelector('.contact_row_info_gsap')) {
            contactRow.classList.add("contact_row_hidden");

            document.querySelector('.contact_row_info_gsap').style.pointerEvents = 'none';
        }

        document.getElementById('contact_form_submit').classList.add('contact_row_form_block_submit_success')
        ...
        setTimeout(() => {
            document.getElementById('contact_form_submit').classList.remove('contact_row_form_block_submit_success');
            ...
            success: function () {
                ...
                setTimeout(() => {
                    ...
                    if (document.querySelector('.contact_row_info_gsap')) {
                        ...
                        document.querySelector('.contact_row_info_gsap').style.pointerEvents = 'visible';
                    }
                }, 5000);
            },
        });
