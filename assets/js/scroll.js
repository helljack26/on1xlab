...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let sections = gsap.utils.toArray("section"),
        ...
        scrollToContact = !isProgectPage ? gsap.utils.toArray("#scroll_to_Contact") : undefined,
        getMaxWidth = () =>
    ...
    const setActiveLink = (progress) => {
        ...
        const magneticLink = document.querySelectorAll(".bottom_nav_link_magnetic_text");
        ...
        for (let i = 1; i <= 6; i++) {
            if (currentSlide == 2 && !isProgectPage) {
                ...
                document.querySelector('.tooltip__trigger').classList.add('invert_filter');
            } else {
                $('.header_social_link').removeClass('invert_filter');
                document.querySelector('.tooltip__trigger').classList.remove('invert_filter');
            }

            if (magneticLink[i - isMinusOneForIndex] !== undefined) {
                if (i - isMinusOneForIndex <= currentSlide - isMinusOneForProject) {
                    magneticLink[i - isMinusOneForIndex].classList.add('bottom_nav_link_active');
                } else {
                    magneticLink[i - isMinusOneForIndex].classList.remove('bottom_nav_link_active');
                }
            }
        }
    };
    ...
    gsap.to(progressBar, {
        x: () =>
            -(progressBar.scrollWidth - document.documentElement.clientWidth) + "px",
        ease: "none",
        scrollTrigger: {
            trigger: progressBar,
            start: "top top",
    ...
    document.querySelector('.contact_row_info').addEventListener('mouseover', () => {
        document.querySelector('.contact_map').classList.add('small_contact_map');
    });

    document.querySelector('.contact_row_info').addEventListener('mouseout', () => {
        document.querySelector('.contact_map').classList.remove('small_contact_map');
    });
